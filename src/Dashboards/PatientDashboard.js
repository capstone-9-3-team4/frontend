...
//import PatientJournalCard from "../Components/Patients/PatientJournalCard";
 ...
 const AI_APIKEY = process.env.REACT_APP_OPENAI_API_KEY;
  ...
  const handleSubmit = async (e) => {
   ...
   if (entryText) {
    const resp = await openai.call(`${entryText}, Based on this journal entry, can you give me a response and rating based on the numbers 1, 2, and 3 (1 being the author is in a high risk level concern for their mental health state, 2 being the author is in a moderate level risk concern for their mental health state and 3 being the author is in a good and positive mental health state). Please evaluate the entry as a psychiatrist using professional medical terminology.`)
