...
//import PatientJournalCard from "../Components/Patients/PatientJournalCard";
 ...
 const AI_APIKEY = process.env.REACT_APP_OPENAI_API_KEY;
  ...
  const handleSubmit = async (e) => {
   ...
   if (entryText) {
    const resp = await openai.call(`${entryText},  based on this entry can you give me a response based on the category 1, 2, and 3 (1 being the patient in a concern menthal health state, 2 being the patient is in a regular mental health state and 3 being the patient has a good mental health state), please evaluate the entry as a psychiatrist using profesional medical terminology,avoid using phrases like 'it appears that the patient is in an overall okay mental health state', please response with only one category and no more than 2 paragraph`)
